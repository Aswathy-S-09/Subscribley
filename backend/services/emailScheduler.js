...
class EmailScheduler {
    ...
    if (hasSmtpCreds) {
      ...
      this.transporter.verify((error, success) => {
        if (error) {
          ...
          console.log('📧 Switching to email logging mode');
          this.transporter = null; // Disable SMTP, will use logging fallback
        } else {
          console.log('✅ Email Scheduler SMTP transporter is ready');
        }
      });
    } else {
      console.log('⚠️ SMTP not configured for email scheduler - using logging mode');
    }
  }
  ...
  async sendExpirationEmail(user, subscriptions, urgencyLevel) {
    ...
    try {
      const emailContent = this.generateEmailContent(user, subscriptions, urgencyLevel);
